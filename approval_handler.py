...
def approve_and_group_quotes(db: Session):
    ...
    # Process quotes in batches for grouping
    for i in range(0, len(all_pending_quotes), GROUPING_BATCH_SIZE):
        batch_quotes = all_pending_quotes[i:i + GROUPING_BATCH_SIZE]
        print(f"Processing grouping batch {int(i/GROUPING_BATCH_SIZE) + 1}/{(len(all_pending_quotes) + GROUPING_BATCH_SIZE - 1) // GROUPING_BATCH_SIZE} with {len(batch_quotes)} quotes.")
        ...
        try:
            ...
            if response is None:
                print(f"Warning: LLM returned no response for grouping batch {int(i/GROUPING_BATCH_SIZE) + 1}. Skipping this batch.")
                continue

            grouped_ids_lists_batch = json.loads(response)

            if not isinstance(grouped_ids_lists_batch, list):
                print(f"Warning: LLM returned non-list for grouping batch {int(i/GROUPING_BATCH_SIZE) + 1}. Skipping grouping for this batch.")
            ...
            print(f"Finished grouping batch {int(i/GROUPING_BATCH_SIZE) + 1}. Quotes within groups are now APPROVED.")

        except json.JSONDecodeError:
            print(f"Error: LLM returned invalid JSON for grouping batch {int(i/GROUPING_BATCH_SIZE) + 1}. Skipping grouping for this batch.")
            db.rollback()
        except Exception as e:
            print(f"An error occurred during grouping batch {int(i/GROUPING_BATCH_SIZE) + 1}: {e}")
            db.rollback()
